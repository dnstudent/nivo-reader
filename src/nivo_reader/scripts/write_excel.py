...
def write_excel_workbook(
    ...
    Args:
        long_df: The input digitization dataframe (long format)
    ...
    pd_df = (
        long_df.pivot(on="column", index="row", values="content", sort_columns=True)
        .sort(by="row")
        .drop("row")
        .to_pandas()
    )
    ...
    num_rows = long_df.get_column("row").n_unique()
    num_cols = long_df.get_column("column").n_unique()
    table_region = (row + 1, col + 1, row + num_rows, col + num_cols)

    # Apply styling
    styler.apply(ws, long_df, table_region)
...
def write_excels_batch(
    ...
    digitization_files = discover_files(input_dir, "*.arrow")
    ...
    for digitization_file in tqdm(digitization_files, desc="Writing Excel files"):
        ...
        wb = write_excel_workbook(digitization_df, styler, header_path)
...
def create_parser():
    ...
    _ = parser.add_argument(
        "--station-details",
        help="Path to an Excel file containing station details (column 'Stazione' and 'Elevazione')",
        type=str,
        default=None,
    )
    return parser
